...
# KEY customer_id (customer_id): An index (INDEX) has been created for the customer_id column, which improves the performance of searches that use this column.
# Features and benefits of indicators:
# Search speed: Using indexes helps the database find records faster, especially in large or complex searches.
# Optimization operations: indexes can perform optimizations on operations such as SELECT, JOIN, WHERE and ORDER BY.
# Disadvantages:
# Latency in write operations: Indexes must be updated every time a new record is added, edited, or deleted. This may increase the time in the write operation.
# More storage space: Indexes require more space and may increase disk space consumption.
